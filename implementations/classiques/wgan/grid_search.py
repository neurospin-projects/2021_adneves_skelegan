...
_, skel_train, skel_val, _ = main_create('skeleton','L',batch_size = opt.batch_size, nb = 1000,adn=False, directory_base='/neurospin/dico/data/deep_folding/data/crops/STS_branches/nearest/original/Lskeleton')
_, _, _, skel_test = main_create('skeleton','L',batch_size =1, nb = 1000,adn=False, directory_base='/neurospin/dico/data/deep_folding/data/crops/STS_branches/nearest/original/Lskeleton')
for n_critic in range_n_critic:
    for dim_latent in range_dim_latent:
        for lr in range_lr:
            ...
            if os.path.exists(join('/neurospin/dico/adneves/grid_search_80',name_dir)):
                shutil.rmtree(join('/neurospin/dico/adneves/grid_search_80',name_dir))
            os.makedirs(join('/neurospin/dico/adneves/grid_search_80',name_dir), exist_ok=True)
            try:
                os.mkdir(join('/neurospin/dico/adneves/grid_search_80',name_dir, 'images'))
            except:
            ...
            class Encoder(nn.Module):
                def __init__(self,batch_size,latent_dim, in_channels=1, dim=8, n_downsample=3):
                    ...
                    self.l1 = nn.Sequential(nn.Linear(23328, self.latent_dim))
            ...
            for epoch in range(1,opt.n_epochs+1):
                for batch_skel in skel_train:
                    ...
                    if i % opt.sample_interval == 0:
                        ...
                        save_image(image_grid, "/neurospin/dico/adneves/grid_search_80/%s/%s/%s.png" % (name_dir,"images","target" + str(epoch) + '_' + str(i)), nrow=5, normalize=True)
                        ...
                        save_image(fake_grid, "/neurospin/dico/adneves/grid_search_80/%s/%s/%s.png" % (name_dir,"images","fake_img" + str(epoch) + '_' + str(i)), nrow=5, normalize=True)
            ...
            name=join('/neurospin/dico/adneves/grid_search_80/', 'epoch_'+str(epoch+1)+'_'+ str(name_dir)+ '_checkpoint.pth.tar')
            ...
            if opt.generation !=0:
                ...
                for new_im in range(opt.generation):
                    ...
                    fake_imgs4 = make_grid(fake_maxed[0,60,:,:], nrow=5, normalize=True)
                    fake_grid = torch.cat((fake_imgs1, fake_imgs2, fake_imgs3, fake_imgs4), 1)
                    save_image(fake_grid, "/neurospin/dico/adneves/grid_search_80/%s/%s/%s.png" % (name_dir,"images","new_im_" + str(new_im)), nrow=5, normalize=True)
                    gen_n += 1
            ...
            np.save("/neurospin/dico/adneves/grid_search_80/%s/vectmoyen.npy" % (name_dir), gen_max)
